...
{
    throw "Не найдена папка tests: $testsRoot"
}
...
function Ensure-Dir($path)
{
    if (-not (Test-Path $path))
    {
        if ($DryRun)
        {
            Write-Host "[DRY] MKDIR $path"; return
        }
...
function Move-File($src, $dst)
{
    ...
    if ($DryRun)
    {
        Write-Host "[DRY] MOVE $src -> $dst"; return
    }
...
function Update-Namespace($filePath, $ns, $skip = $false)
{
    ...
    if ([IO.Path]::GetFileName($filePath) -ieq 'bootstrap.php')
    {
        return
    }
    ...
    if ($DryRun)
    {
        Write-Host "[DRY] NS  $filePath -> $ns"; return
    }
