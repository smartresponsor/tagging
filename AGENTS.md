...
The contour has two distinct roles:

- `Objecting`, `Cruding`, `Viewing`, and `Interfacing` are real application dependencies. They are declared explicitly through Composer and may resolve to local symlink path repositories in development or package/bundle dependencies in production.
- `Gating` and `Canonization` are mandatory read-and-comply contract repositories. Before implementation conclusions or changes, read and comply with each available `AGENTS.md`, `README.md`, `composer.json`, manifest, local gate configuration, and linked canonical contract. They must not be invented as runtime Composer dependencies unless they later expose and the application actually consumes a real package surface.
