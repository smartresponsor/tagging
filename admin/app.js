...
(function(){
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if (e.ctrlKey && e.key === '1'){ e.preventDefault(); activate('search'); }
    if (e.ctrlKey && e.key === '2'){ e.preventDefault(); activate('assign'); }
    if (e.ctrlKey && e.key === '3'){ e.preventDefault(); activate('synonym'); }
  });
  ...
  async function call(method, path, body){
    ...
    const resp = await fetch(url, { method, headers, body: (method==='GET'||method==='DELETE')? undefined : bodyStr });
