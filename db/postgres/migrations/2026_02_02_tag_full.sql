...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name='tag_entity' AND column_name='required_flag'
  ) THEN
    ALTER TABLE tag_entity ADD COLUMN required_flag BOOLEAN NOT NULL DEFAULT FALSE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name='tag_entity' AND column_name='mod_only_flag'
  ) THEN
    ALTER TABLE tag_entity ADD COLUMN mod_only_flag BOOLEAN NOT NULL DEFAULT FALSE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'tag_entity_set_updated_at'
  ) THEN
    CREATE TRIGGER tag_entity_set_updated_at
    BEFORE UPDATE ON tag_entity
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tag_synonym_tag_fk'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tag_relation_from_fk'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tag_relation_to_fk'
  ) THEN
    ALTER TABLE tag_relation
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tag_redirect_to_fk'
  ) THEN
