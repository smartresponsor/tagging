window.__TENANT__ = undefined;
window.__TAG_BASE__ = undefined;
(function () {
  ...
  if (card) {
    ...
    fetch(base + '/tag/' + encodeURIComponent(id), {headers: h('GET')})
      .then(r => r.json()).then(d => {
      card.textContent = JSON.stringify(d, null, 2);
    })
      .catch(e => {
        card.textContent = 'Error: ' + e;
      });
  }
  ...
  if (purge) {
    purge.addEventListener('click', function () {
      ...
      fetch(base + '/tag/_purge', {
        method: 'POST', headers: Object.assign({'Content-Type': 'application/json'}, h('POST')),
        body: JSON.stringify({action: 'tag_ids', tag_ids: [id]})
      })
        .then(r => r.json()).then(d => {
        alert('Purged: ' + JSON.stringify(d));
      })
  ...
  if (assignForm) {
    assignForm.addEventListener('submit', function (ev) {
      ...
      fetch(base + '/tag/' + encodeURIComponent(id) + '/assign', {
        ...
        document.querySelector('#assign-result').textContent = 'Error: ' + e;
      });
    });
  }
