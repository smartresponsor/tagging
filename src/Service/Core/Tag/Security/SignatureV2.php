...
namespace App\Service\Core\Tag\Security;
...
final class NonceCache
{
    ...
    public static function configTtl(int $ttl): void
    {
        self::$ttl = max(60, $ttl);
    }
...
final readonly class SignatureV2
{
    ...
    public function verify(string $method, string $path, string $body, array $headers): void
    {
        ...
        $ok = false;
        if ('' !== $cur && hash_equals($this->h($data, $cur), $hSig)) {
            $ok = true;
        }
        if (!$ok && '' !== $prv && hash_equals($this->h($data, $prv), $hSig)) {
            $ok = true;
        }
        ...
        TagMetrics::inc('tag_sig_ok_total');
    }
