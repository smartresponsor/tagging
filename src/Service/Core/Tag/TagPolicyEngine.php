...
final readonly class TagPolicyEngine
{
    ...
    public function validateTag(string $slug, string $label): void
    {
        ...
        if (mb_strlen($slug) > $max || mb_strlen($label) > $max) {
            throw new \InvalidArgumentException('policy.maxLength');
        }
        $allowedSlugs = $this->policy['allowedSlugs'] ?? [];
        if ($allowedSlugs && !in_array($slug, $allowedSlugs, true)) {
            throw new \InvalidArgumentException('policy.allowedSlugs');
        }
        $denied = $this->policy['deniedPrefixes'] ?? [];
        foreach ($denied as $p) {
            if (str_starts_with($slug, $p)) {
                throw new \InvalidArgumentException('policy.deniedPrefixes');
            }
        }
        ...
        if ($allowed) {
            ...
            if (!$ok) {
                throw new \InvalidArgumentException('policy.allowedPrefixes');
            }
        }
    }
}
