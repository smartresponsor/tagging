...
namespace App\Tagging\Service\Core\Webhook;
...
final readonly class TagWebhookSender
{
    ...
    {
        return (string) ($this->cfg['spool_dir'] ?? 'report/webhook/spool');
    }

    private function dlq(): string
    {
        return (string) ($this->cfg['dlq_path'] ?? 'report/webhook/dlq.ndjson');
    }

    private function header(): string
    {
        return (string) ($this->cfg['signature_header'] ?? 'X-SR-Signature');
    }

    private function retries(): int
    {
        return max(0, (int) ($this->cfg['retries'] ?? 5));
    }

    private function baseDelay(): int
    {
        return max(1, (int) ($this->cfg['base_delay_ms'] ?? 200));
    }

    private function maxDelay(): int
    {
        return max(1000, (int) ($this->cfg['max_delay_ms'] ?? 10000));
    }

    private function dirMode(): int
    {
        return (int) ($this->cfg['dir_mode'] ?? 0700);
    }
