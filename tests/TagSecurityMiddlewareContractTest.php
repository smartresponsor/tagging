...
use App\Tagging\Http\Api\Tag\Middleware\TagVerifySignatureMiddleware;
use App\Tagging\Ops\Security\TagNonceStore;
use App\Tagging\Service\Security\TagHmacV2Verifier;
use PHPUnit\Framework\TestCase;

final class TagSecurityMiddlewareContractTest extends TestCase
{
    public function testVerifySignatureRejectsMissingHeadersWithStableJsonContract(): void
    {
        $store = new TagNonceStore(sys_get_temp_dir() . '/tag-nonce-' . uniqid('', true), 300, 1000);
        $middleware = new TagVerifySignatureMiddleware(
            new TagHmacV2Verifier('secret', 120, $store),
            [
                'enforce' => true,
                'secret' => 'secret',
