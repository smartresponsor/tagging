...
$PostgresDb = $env:POSTGRES_DB; if (-not $PostgresDb)
{
    $PostgresDb = "app"
}
$PostgresUser = $env:POSTGRES_USER; if (-not $PostgresUser)
{
    $PostgresUser = "app"
}
$PostgresPassword = $env:POSTGRES_PASSWORD; if (-not $PostgresPassword)
{
    $PostgresPassword = "app"
}
$DbHost = $env:DB_HOST; if (-not $DbHost)
{
    $DbHost = "127.0.0.1"
}
$DbPort = $env:DB_PORT; if (-not $DbPort)
{
    $DbPort = "5432"
}
...
for ($i = 0; $i -lt 40; $i++) {
    try
    {
        Invoke-Psql "select 1"; $ok = $true; break
    }
    catch
    {
